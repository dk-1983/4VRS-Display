...
inline const char *haPolicy(bool enabled,uint32_t revision) {
  if(settings.managed&&(revision<settings.revision||(revision==settings.revision&&enabled!=settings.ha)))return "policy_revision";
  Settings next=settings;next.ha=enabled;next.managed=true;next.revision=revision;
  if((!settings.managed||revision!=settings.revision)&&!saveSettings(next))return "policy_storage";
  leaseUntil=millis()+90000;++changes;return nullptr;
}
...
inline bool decode(const char *text,size_t size,Manifest &m) {
  cJSON *envelope=RackMqtt::parse(text,size);if(!envelope)return false;
  ...
  if(cJSON_IsString(payload)&&cJSON_IsString(signature)&&strlen(payload->valuestring)<=2048&&strlen(signature->valuestring)<=144) {
    ...
    if(sigSize>=64&&unhex(signature->valuestring,sig,sigSize)&&mbedtls_pk_parse_public_key(&key,(const unsigned char*)UPDATE_PUBLIC_KEY,strlen(UPDATE_PUBLIC_KEY)+1)==0&&mbedtls_sha256((const unsigned char*)payload->valuestring,strlen(payload->valuestring),digest,0)==0&&mbedtls_pk_verify(&key,MBEDTLS_MD_SHA256,digest,32,sig,sigSize)==0) {
      cJSON *p=RackMqtt::parse(payload->valuestring,strlen(payload->valuestring));char profile[48],channel[16];uint32_t schema;
      if(p){ok=RackMqtt::numberField(p,"schema",schema,1,1)&&RackMqtt::textField(p,"profile",profile,sizeof(profile))&&!strcmp(profile,PROFILE)&&RackMqtt::textField(p,"channel",channel,sizeof(channel))&&!strcmp(channel,"stable")&&RackMqtt::textField(p,"version",m.version,sizeof(m.version))&&RackMqtt::textField(p,"url",m.url,sizeof(m.url))&&!strncmp(m.url,ASSET_PREFIX,strlen(ASSET_PREFIX))&&RackMqtt::textField(p,"sha256",m.sha,sizeof(m.sha))&&RackMqtt::numberField(p,"size",m.size,128,0x1e0000);cJSON_Delete(p);}
...
inline bool begin(const char *version) {
  ...
  storageReady=storage.begin("rack-update",false);if(!storageReady)return false;
  ...
  bootStart=millis();bootConfirmed=!pendingBoot;state(pendingBoot?"confirming_boot":"idle");
  return xTaskCreate(worker,"rack-github",12288,nullptr,1,nullptr)==pdPASS;
}
inline void tick(bool localHealthy,bool connected) {
  ...
  if(pendingBoot){
    ...
    if(healthySince&&millis()-healthySince>=45000){
      if(esp_ota_mark_app_valid_cancel_rollback()==ESP_OK){pendingBoot=false;bootConfirmed=true;state("idle");}
    } else if(millis()-bootStart>120000)esp_ota_mark_app_invalid_rollback_and_reboot();
  }
